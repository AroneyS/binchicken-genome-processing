  ...
  genome_across %>%
    ...
    mutate(
      coverage_factor = case_when(
        trimmed_mean_coverage < 5 ~ "<5",
        trimmed_mean_coverage < 10 ~ "<10",
        trimmed_mean_coverage < 20 ~ "<20",
        trimmed_mean_coverage < 50 ~ "<50",
        TRUE ~ ">=50"
        ),
      coverage_factor = factor(coverage_factor, levels = c("<5", "<10", "<20", "<50", ">=50")),
      ) %>%
