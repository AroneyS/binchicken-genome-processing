...
protein_bc_non_singletons <- read_tsv(here("binchicken_RBGs", "metabolism", "binchicken_unique_annotations.tsv"))
bc_non_singletons <- read_tsv(here("binchicken_RBGs", "metabolism", "binchicken_unique_list.tsv"), col_names = c("cluster")) %>%
  left_join(protein_bc_non_singletons, by = c("cluster" = "#query"))
```
...
genome_rnas_plot <- cowplot::plot_grid(
...
ggsave(here(output_dir, "genome_quality_rnas.png"), dpi = 600, width = 12, height = 14)
ggsave(here(output_dir, "genome_quality_rnas.svg"), dpi = 600, width = 12, height = 14)
```
...
ggsave(here(output_dir, "metagroups.svg"), dpi = 600, width = 180, height = 60, units = "mm")
```
...
ggsave(here(output_dir, "target_sequences.svg"), dpi = 600, height = 7, width = 12)
...
genome_teas_stats <- genome_teas_pathways %>%
  ...
  mutate(
    ...
    df = map_dbl(tidy_anova, ~ .x$df[[2]]),
    deviance = map_dbl(tidy_anova, ~ .x$deviance[[2]]),
...
genome_teas_stats %>%
  select(tea_group, df, deviance, pval, adj_pval, stars, max_fold_change, max_change) %>%
  write_csv(here(output_dir, "genome_teas_stats.csv"))
...
proteins_teas_pathways_lines <- genome_teas_pathways %>%
  ...
  geom_line() +
  geom_point() +
  geom_text(aes(label = label), data = genome_teas_stats %>% filter(adj_pval < 0.05), nudge_y = 0.03, size = 5 / .pt) +
...
### Novel clusters ###
######################
novel_proteins <- bc_non_singletons %>%
  mutate(
    KEGG_ko = ifelse(KEGG_ko == "-", NA_character_, KEGG_ko),
    EC = ifelse(EC == "-", NA_character_, EC),
    PFAMs = ifelse(PFAMs == "-", NA_character_, PFAMs),
    CAZy = ifelse(CAZy == "-", NA_character_, CAZy),
    BiGG_Reaction = ifelse(BiGG_Reaction == "-", NA_character_, BiGG_Reaction),
    COG_category = ifelse(COG_category == "-", NA_character_, COG_category),
    novelty = case_when(
      is.na(seed_ortholog) ~ "No hits",
      !is.na(KEGG_ko) | !is.na(EC) | !is.na(CAZy) | !is.na(BiGG_Reaction) ~ "Known enzyme",
      !is.na(PFAMs) ~ "Known domain",
      !is.na(COG_category) ~ "Known COG category",
      TRUE ~ "Uncharacterised",
      )
    ) %>%
  group_by(novelty) %>%
  count() %>%
  ggplot(aes(reorder(novelty, n), n)) +
  geom_col() +
  coord_flip() +
  scale_y_continuous("Count", labels = ~ format(.x, scientific = FALSE)) +
  scale_x_discrete("Novelty") +
  theme_cowplot()
novel_proteins
ggsave("novel_proteins.png", path = output_dir, dpi = 600, height = 7, width = 12)

cog_categories <- tribble(
  ~COG_category, ~Description, ~Short_Description,
  "A", "RNA processing and modification", "RNA (A)",
  "B", "Chromatin structure and dynamics", "Chromatin (B)",
  "C", "Energy production and conversion", "Energy (C)",
  "D", "Cell cycle control, cell division, chromosome partitioning", "Cell cycle (D)",
  "E", "Amino acid transport and metabolism", "Amino acid (E)",
  "F", "Nucleotide transport and metabolism", "Nucleotide (F)",
  "G", "Carbohydrate transport and metabolism", "Carbohydrate (G)",
  "H", "Coenzyme transport and metabolism", "Coenzyme (H)",
  "I", "Lipid transport and metabolism", "Lipid (I)",
  "J", "Translation, ribosomal structure and biogenesis", "Translation (J)",
  "K", "Transcription", "Transcription (K)",
  "L", "Replication, recombination and repair", "Replication (L)",
  "M", "Cell wall/membrane/envelope biogenesis", "Cell wall (M)",
  "N", "Cell motility", "Cell motility (N)",
  "O", "Posttranslational modification, protein turnover, chaperones", "Posttranslational (O)",
  "P", "Inorganic ion transport and metabolism", "Inorganic ion (P)",
  "Q", "Secondary metabolites biosynthesis, transport and catabolism", "Secondary metabolites (Q)",
  "R", "General function prediction only", "General (R)",
  "S", "Function unknown", "Unknown (S)",
  "T", "Signal transduction mechanisms", "Signal transduction (T)",
  "U", "Intracellular trafficking, secretion, and vesicular transport", "Intracellular trafficking (U)",
  "V", "Defense mechanisms", "Defense (V)",
  "W", "Extracellular structures", "Extracellular (W)",
  "X", "nan", "nan (X)",
  "Y", "Nuclear structure", "Nuclear (Y)",
  "Z", "Cytoskeleton", "Cytoskeleton (Z)",
  "NU", "Cell motility and/or Intracellular trafficking, secretion, and vesicular transport", "Motility / Trafficking (NU)",
)

novel_cogs <- bc_non_singletons %>%
  replace_na(list(COG_category = "-")) %>%
  mutate(COG_category = ifelse(COG_category == "-", "Unknown", COG_category)) %>%
  filter(COG_category != "Unknown") %>%
  group_by(COG_category) %>%
  count() %>%
  arrange(desc(n)) %>%
  head(20) %>%
  left_join(cog_categories, by = "COG_category") %>%
  ggplot(aes(reorder(Short_Description, n), n)) +
  geom_col() +
  coord_flip() +
  scale_y_continuous("Count") +
  scale_x_discrete("COG category (top 20)") +
  theme_cowplot()
novel_cogs
ggsave("novel_cogs.png", path = output_dir, dpi = 600, height = 7, width = 12)

novel_name <- bc_non_singletons %>%
  replace_na(list(Preferred_name = "-")) %>%
  mutate(Preferred_name = ifelse(Preferred_name == "-", "Unknown", Preferred_name)) %>%
  filter(Preferred_name != "Unknown") %>%
  group_by(Preferred_name) %>%
  count() %>%
  arrange(desc(n)) %>%
  head(20) %>%
  ggplot(aes(reorder(Preferred_name, n), n)) +
  geom_col() +
  coord_flip() +
  scale_y_continuous("Count") +
  scale_x_discrete("Gene name (top 20)") +
  theme_cowplot()
novel_name
ggsave("novel_name.png", path = output_dir, dpi = 600, height = 7, width = 12)

kegg_labels <- tribble(
  ~KEGG_ko, ~KEGG_label,
  "ko:K03088", "rpoE (K03088)",
  "ko:K01992", "ABC permease protein (K01992)",
  "ko:K01990", "ABC ATP binding (K01990)",
  "ko:K02004", "ABC permease protein (K02004)",
  "ko:K12132", "Serine/threonine kinase (K12132)",
  "ko:K03406", "Methyl-accepting chemotaxis (K03406)",
  "ko:K06147", "ABC subfamily B (K06147)",
  "ko:K02003", "ABC ATP binding (K02003)",
  "ko:K02035", "ABC substrate binding (K02035)",
  "ko:K02014", "Iron complex outer membrane (K02014)",
  "ko:K01768", "Adenylate cyclase (K01768)",
  "ko:K03687", "Chaperone grpE (K03687)",
  "ko:K07090", "Uncharacterised (K07090)",
  "ko:K01869", "Leucyl-tRNA synthase (K01869)",
  "ko:K02529", "Galactose operon repressor (K02529)",
  "ko:K03046", "rpoC (K03046)",
  "ko:K07052", "CAAX protease (K07052)",
  "ko:K07114", "Ca-activ. chloride channel (K07114)",
  "ko:K03043", "rpoB (K03043)",
  "ko:K01873", "Valyl-tRNA synthase (K01873)",
  "ko:K03657", "ATP-dep. DNA helicase (K03657)",
  "ko:K02238", "Competence protein ComEC (K02238)",
  "ko:K03797", "Carboxyl-terminal protease (K03797)",
  "ko:K00059", "3-oxoacyl reductase (K00059)",
  "ko:K04771", "Serine protease Do (K04771)",
  "ko:K03086", "rpoD (K03086)",
  "ko:K13993", "HSP20 family protein (K13993)",
  "ko:K03100", "Signal peptidase I (K03100)",
)

novel_ko <- bc_non_singletons %>%
  replace_na(list(KEGG_ko = "-")) %>%
  mutate(KEGG_ko = ifelse(KEGG_ko == "-", "Unknown", KEGG_ko)) %>%
  filter(KEGG_ko != "Unknown") %>%
  group_by(KEGG_ko) %>%
  count() %>%
  arrange(desc(n)) %>%
  head(20) %>%
  left_join(kegg_labels, by = "KEGG_ko") %>%
  ggplot(aes(reorder(KEGG_label, n), n)) +
  geom_col() +
  coord_flip() +
  scale_y_continuous("Count") +
  scale_x_discrete("KEGG ko ID (top 20)") +
  theme_cowplot()
novel_ko
ggsave("novel_ko.png", path = output_dir, dpi = 600, height = 7, width = 12)

novel_kegg_module <- bc_non_singletons %>%
  replace_na(list(KEGG_Module = "-")) %>%
  mutate(KEGG_Module = ifelse(KEGG_Module == "-", "Unknown", KEGG_Module)) %>%
  filter(KEGG_Module != "Unknown") %>%
  group_by(KEGG_Module) %>%
  count() %>%
  arrange(desc(n)) %>%
  head(20) %>%
  ggplot(aes(reorder(KEGG_Module, n), n)) +
  geom_col() +
  coord_flip() +
  scale_y_continuous("Count") +
  scale_x_discrete("KEGG module (top 20)") +
  theme_cowplot()
novel_kegg_module
ggsave("novel_kegg_module.png", path = output_dir, dpi = 600, height = 7, width = 12)

novel_pfam <- bc_non_singletons %>%
  replace_na(list(PFAMs = "-")) %>%
  mutate(PFAMs = ifelse(PFAMs == "-", "Unknown", PFAMs)) %>%
  filter(PFAMs != "Unknown") %>%
  group_by(PFAMs) %>%
  count() %>%
  arrange(desc(n)) %>%
  head(20) %>%
  ggplot(aes(reorder(PFAMs, n), n)) +
  geom_col() +
  coord_flip() +
  scale_y_continuous("Count") +
  scale_x_discrete("PFAM (top 20)") +
  theme_cowplot()
novel_pfam
ggsave("novel_pfam.png", path = output_dir, dpi = 600, height = 7, width = 12)

cowplot::plot_grid(
  novel_proteins + theme_cowplot(font_size = 7),
  novel_cogs + theme_cowplot(font_size = 7),
  novel_ko + theme_cowplot(font_size = 7),
  novel_pfam + theme_cowplot(font_size = 7),
  nrow = 2,
  labels = "auto", label_size = 10,
  align = "h", axis = "lrtb"
)

ggsave("novel_proteins_comb.png", path = output_dir, dpi = 600, width = 180, height = 120, units = "mm")
ggsave("novel_proteins_comb.svg", path = output_dir, dpi = 600, width = 180, height = 120, units = "mm")
```
...
```{r novelty-size}
novel_genome_metadata <- genome_metadata %>%
  select(name, rep = `95ANI_representative`) %>%
  filter(!is.na(rep)) %>%
  left_join(named_taxonomy %>% select(rep = name, named_taxonomy)) %>%
  mutate(
    domain = str_extract(named_taxonomy, "d__[^;]+"),
    named_novelty = case_when(
      str_detect(named_taxonomy, "p__binchicken") ~ "Phylum novelty",
      str_detect(named_taxonomy, "c__binchicken") ~ "Class novelty",
      str_detect(named_taxonomy, "o__binchicken") ~ "Order novelty",
    ...
    novel_phylum = str_extract(named_taxonomy, "p__binchicken[^;]+"),
    novel_class = str_extract(named_taxonomy, "c__binchicken[^;]+"),
    novel_order = str_extract(named_taxonomy, "o__binchicken[^;]+"),
    novel_family = str_extract(named_taxonomy, "f__binchicken[^;]+"),
    novel_genus = str_extract(named_taxonomy, "g__binchicken[^;]+"),
    novel_species = str_extract(named_taxonomy, "s__.*"),
    )

# Phylum
novel_genome_metadata %>%
  filter(!is.na(novel_phylum)) %>%
  group_by(domain, novel_phylum, rep) %>% count() %>%
  group_by(domain, novel_phylum) %>%
  count() %>% 
  filter(n > 1) %>% 
  group_by(domain) %>% 
  count()
...
# Class
novel_genome_metadata %>%
  filter(!is.na(novel_class)) %>%
  group_by(domain, novel_class, rep) %>% count() %>%
  group_by(domain, novel_class) %>%
  count() %>% 
  filter(n > 1) %>% 
  group_by(domain) %>% 
  count()
...
# Order
novel_genome_metadata %>%
  filter(!is.na(novel_order)) %>%
  group_by(domain, novel_order, rep) %>% count() %>%
  group_by(domain, novel_order) %>%
  count() %>% 
  filter(n > 1) %>% 
  group_by(domain) %>% 
  count()
...
# Family
novel_genome_metadata %>%
  filter(!is.na(novel_family)) %>%
  group_by(domain, novel_family, rep) %>% count() %>%
  group_by(domain, novel_family) %>%
  count() %>% 
  filter(n > 1) %>% 
  group_by(domain) %>% 
  count()
...
# Genus
novel_genome_metadata %>%
  filter(!is.na(novel_genus)) %>%
  group_by(domain, novel_genus, rep) %>% count() %>%
  group_by(domain, novel_genus) %>%
  count() %>% 
  filter(n > 1) %>% 
  group_by(domain) %>% 
  count()
...
# Species
novel_genome_metadata %>%
  filter(!is.na(novel_species)) %>%
  group_by(domain, novel_species) %>%
  count() %>%
  filter(n > 1) %>%
  group_by(domain) %>%
  count()
