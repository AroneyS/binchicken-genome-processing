...
MEDIAN_RED = pl.DataFrame([
    ["phylum", 0.3280941769231098, 0.2128708845277663],
    ["class", 0.449727838796469, 0.35878546884559126],
    ["order", 0.6083500718998613, 0.5316295929627715],
    ["family", 0.7576141066814935, 0.7250725361353227],
    ["genus", 0.9220350796053899, 0.9069458981600348],
    ["species", 1, 1],
], schema=MEDIAN_RED_COLUMNS)
...
def name_clades(tree_df, genome_metadata, domain):
    """
    Traverse tree bottom up, naming nodes when novelty changes based on the earliest magset that is in the clade
    ...
    Start with list of non-GTDB genomes in order of publication and quality (completeness - 5 * contamination)
    Stop once node reached with GTDB children - note node (fill out remaining taxonomy based on GTDB genomes?)
    ...
    if domain == "d__Bacteria":
        ...
        domain_prefix = "a"
    median_reds = dict(MEDIAN_RED.select("taxon", red = domain).iter_rows())
    ...
    magset_name_counters = {}
    for magset in MAGSET_ORDER:
        magset_name_counters[magset] = {k: v for k, v in starting_counters.items()}

    magset_dict = {val: idx for idx, val in enumerate(MAGSET_ORDER)}
    genomes = (
        tree_df
        ...
        .sort(pl.col("magset").map_dict(magset_dict), pl.col("quality"), pl.col("genome"), descending=[False, True, True])
        .get_column("genome")
        .to_list()
    )
    ...
    tree_genomes = (
        ...
        .with_columns(
            parents = pl.col("node").map_elements(lambda x: find_parents(x))
            )
        .sort("genome_order")
    )
    ...
    magset_index = tree_genomes.columns.index("magset")
    parents_index = tree_genomes.columns.index("parents")
    for row in tree_genomes.iter_rows():
        genome = row[genome_index]
        magset = row[magset_index]
                ...
                if name_child:
                    ...
                    for novelty_red in child[1]:
                        if not genome_taxonomy[novelty_red] and not novelty_red == "species":
                            clade_name = novelty_red[0] + "__" + magset + "_" + domain_prefix + str(magset_name_counters[magset][novelty_red])
                            magset_name_counters[magset][novelty_red] += 1
        ...
        for taxon, clade in genome_taxonomy.items():
            ...
            if not clade and taxon_fill[taxon]:
                if taxon == "species":
                    ...
                    clade_name = taxon[0] + "__" + magset + "_" + domain_prefix + str(magset_name_counters[magset][taxon])
                    magset_name_counters[magset][taxon] += 1
...
def main(arguments):
    ...
    genome_metadata = pl.read_csv(args.metadata, separator="\t")
    ...
    (genome_taxonomy, node_names) = name_clades(tree_df, genome_metadata, args.domain)
